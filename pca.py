...
if __name__ == '__main__':
    data=pd.read_csv('./data/datasetunido2.csv')
#print(data.head(5)) #imprimimos los 5 primeros datos
caracteriticas=data.drop(['INCIDENCIA'],axis=1) #las featurus sin el target
objetivo = data['INCIDENCIA'] #obtenemos el target
caracteriticas = StandardScaler().fit_transform(caracteriticas) #Normalizamnos los datos
X_train,X_test,y_train,y_test =train_test_split(caracteriticas,objetivo,test_size=0.30,random_state=    42)
